...
const SeminarDashboard = () => {
  ...
  const [reference, setReference] = useState<string | null>(null);
  const [startPolling, setStartPolling] = useState<boolean>(false);
  const RecordPayment = useCheckoutStore((state) => state.RecordPayment);
  const [userData, setUserData] = useState({
    email: "",
    fullName: "",
    phone: "",
  ...
  // Record payment details to backend
  const recordPayment = async (paymentData: any) => {
    try {
      ...
      const response = await RecordPayment(paymentRecord);
  ...
  // NEW: Direct Paystack payment using public key
  const handlePaystackPayment = () => {
    ...
    try {
      ...
      const paymentConfig = {
        ...
        email: userData.email,
        amount: seminarData.price * 100, // Convert to kobo
        ref: `SEM-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        metadata: {
          custom_fields: [
            {
        ...
        callback: async (response: any) => {
          ...
          // Record payment and handle success
          ...
          if (paymentRecorded) {
            ...
            //await sendSeminarEmail(userData.email, userData.fullName);
            ...
            toast.error(
              <ToastComponent
                ...
                body="Payment was successful but failed to record. Please contact support."
              />,
              { progress: undefined }
            );
          }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      {/* Polling Component for backup verification */}
      {startPolling && reference != null && 
        <PollingComponent 
          paymentReference={reference} 
          setHasPaid={handlePaymentSuccess}
...
// Payment Button Component remains the same
const PaymentButton = ({ 
  handlePayment, 
  handleFormPayment,
  isProcessing, 
